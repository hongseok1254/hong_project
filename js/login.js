...
function showSection(section) {
  ...
  document.querySelectorAll('.sidebar-btn')[btnMap[section]].classList.add('active');
...
let inventoryData = [
  { code: 'A1001', name: '노트북', qty: 120, location: 'A-01', dept: 'IT', position: '사원' },
  { code: 'B2032', name: '마우스', qty: 340, location: 'B-12', dept: '구매', position: '대리' },
  { code: 'C3300', name: '키보드', qty: 210, location: 'C-07', dept: 'IT', position: '과장' },
  { code: 'D4002', name: '모니터', qty: 55, location: 'D-03', dept: '물류', position: '부장' }
];

let ordersData = [
  { no: '20240601-01', customer: '홍길동', status: '대기', date: '2024-06-01' },
  { no: '20240601-02', customer: '이순신', status: '처리중', date: '2024-06-01' },
  { no: '20240601-03', customer: '김철수', status: '완료', date: '2024-06-01' }
];
...
window.openInventoryEditModal = function(idx) {
  ...
  document.getElementById('edit-location').value = inventoryData[idx].location;
  document.getElementById('edit-dept').value = inventoryData[idx].dept;
  document.getElementById('edit-position').value = inventoryData[idx].position;
}
window.closeInventoryModal = function() {
  document.getElementById('inventory-edit-modal').classList.add('hidden');
}
document.getElementById('inventory-edit-form').addEventListener('submit', function(e) {
  e.preventDefault();
  const idx = document.getElementById('edit-index').value;
  inventoryData[idx].name = document.getElementById('edit-name').value;
  inventoryData[idx].qty = parseInt(document.getElementById('edit-qty').value, 10);
  inventoryData[idx].location = document.getElementById('edit-location').value;
  inventoryData[idx].dept = document.getElementById('edit-dept').value;
  inventoryData[idx].position = document.getElementById('edit-position').value;
  renderInventoryTable();
  closeInventoryModal();
});
...
document.getElementById('inventory-add-form').addEventListener('submit', function(e) {
  e.preventDefault();
...
function renderDashboard() {
  ...
  window.mainBarChartInstance = new Chart(barCtx, {
    type: 'bar',
    data: {
      labels: ['1월 27일', '1월 28일', '1월 29일', '1월 30일', '1월 31일', '2월 1일', '2월 2일'],
      datasets: [
        {
          label: '입고',
          data: [2, 1, 3, 2, 6, 1, 5],
  ...
  alertList.innerHTML = '';
  [
    { code: '201801190005', desc: '와이드 롱블랙 라이트 베이지', status: '배송준비', date: '1월 19일' },
    { code: '201801190001', desc: '다다른 롱블랙 라이트 베이지', status: '입금대기', date: '1월 19일' }
  ].forEach(item => {
  ...
  [
    { title: '배송준비', desc: '201801190005 주문이 배송 준비 중입니다.', link: '#' },
    { title: '입금대기', desc: '201801190001 주문이 입금 대기 중입니다.', link: '#' }
  ].forEach(item => {
  ...
  [
    { title: 'LINDA 쇼핑몰 이용방법', desc: '쇼핑몰 이용에 대한 자세한 안내입니다.', link: '#' },
    { title: '반품/교환 방법', desc: '반품 및 교환 절차를 안내합니다.', link: '#' }
  ].forEach(item => {
    const card = document.createElement('div');
    card.className = 'detail-card';
  ...
  [
    { title: '대시보드 업데이트', desc: '8차 대시보드 업데이트가 적용되었습니다.', link: '#' },
    { title: 'SMS 정책 변경', desc: 'SMS 정책이 11월 30일부로 변경됩니다.', link: '#' }
  ].forEach(item => {
    const card = document.createElement('div');
    card.className = 'detail-card';
